...
public class TwoPlayerGame implements Game {
    ...
    private ChatPanel chatPanelPlayerOne;
    private ChatPanel chatPanelPlayerTwo;

    //Constructor takes two players and the server (to get words from)
    public TwoPlayerGame(Player p1, Player p2, Server s, ChatPanel chatPanelPlayerOne, ChatPanel chatPanelPlayerTwo) {
        this.chatPanelPlayerOne = chatPanelPlayerOne;
        this.chatPanelPlayerTwo = chatPanelPlayerTwo;
        server = s;
        guesses = new HashMap<>();
        active = true;
    
        // Assigning player objects passed as parameters to playerOne and playerTwo variables
        playerOne = p1;
        playerTwo = p2;
    ...
    public void sendChatMessage(String playerName, String message) {
        if (playerName.equals(playerOne.getName())) {
            ...
            chatPanelPlayerOne.appendMessage(playerName + ": " + message); // Display the message in the other player's chat panel
        }
        // Send the message to the other player
        // You'll need to access the appropriate Player object(s) and call sendChatMessage()
    }
